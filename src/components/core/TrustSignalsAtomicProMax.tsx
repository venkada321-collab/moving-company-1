...
export const TrustSignalsAtomicProMax: React.FC = () => {
  ...
  if (variant === 'stats-ribbon' || variant === 'stats-ribbon-ticker') {
    return (
      <section className="py-20 bg-neutral-950 text-white relative overflow-hidden border-t border-b border-neutral-800">
        <div className="max-w-7xl mx-auto px-6 mb-16">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-16">
            <div className="card-atomic-surface !bg-neutral-900/90 !p-8 text-center border !border-neutral-800 rounded-3xl hover:border-primary-500/50 transition-all">
              <div className="text-4xl sm:text-5xl font-black text-primary-400 mb-2">4.9★</div>
              <div className="text-xs font-bold text-neutral-400 uppercase tracking-wider">Verified Rating ({LEGAL.reviewCount}+ reviews)</div>
            </div>
            <div className="card-atomic-surface !bg-neutral-900/90 !p-8 text-center border !border-neutral-800 rounded-3xl hover:border-primary-500/50 transition-all">
              <div className="text-4xl sm:text-5xl font-black text-emerald-400 mb-2">{LEGAL.totalMoves}+</div>
              <div className="text-xs font-bold text-neutral-400 uppercase tracking-wider">Flawless Relocations</div>
            </div>
            <div className="card-atomic-surface !bg-neutral-900/90 !p-8 text-center border !border-neutral-800 rounded-3xl hover:border-primary-500/50 transition-all">
              <div className="text-4xl sm:text-5xl font-black text-white mb-2">100%</div>
              <div className="text-xs font-bold text-neutral-400 uppercase tracking-wider">WSIB &amp; COI Insured</div>
            </div>
            <div className="card-atomic-surface !bg-neutral-900/90 !p-8 text-center border !border-neutral-800 rounded-3xl hover:border-primary-500/50 transition-all">
              <div className="text-4xl sm:text-5xl font-black text-primary-300 mb-2">$0</div>
              <div className="text-xs font-bold text-neutral-400 uppercase tracking-wider">Transit Damage Deductible</div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {TESTIMONIALS.slice(0, 3).map((t, idx) => (
              <div key={idx} className="p-6 flex flex-col justify-between bg-neutral-950/80 rounded-2xl border border-neutral-800 shadow-xl">
                <div>
                  <div className="flex text-amber-400 mb-4">{'★'.repeat(t.rating)}</div>
                  <p className="text-sm text-neutral-300 leading-relaxed italic mb-6">&ldquo;{t.text}&rdquo;</p>
                </div>
                <div className="flex items-center justify-between border-t border-neutral-800 pt-4">
                  <div>
                    <div className="font-extrabold text-sm text-white">{t.author}</div>
  ...
  if (variant === 'brutalist-monospaced-audit') {
    return (
      <section className="py-20 bg-zinc-900/60 dark:bg-black text-white dark:text-neutral-100 font-mono border-t-4 border-b-4 border-black dark:border-white">
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="mb-12 border-2 border-black dark:border-white divide-y-2 divide-black dark:divide-white">
            <div className="bg-zinc-800 dark:bg-neutral-900 p-4 font-black text-xs uppercase flex justify-between">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x-2 divide-black dark:divide-white text-xs">
              {AWARDS.slice(0, 4).map((award, i) => (
                <div key={i} className="p-5 flex items-center justify-between bg-zinc-900/60 dark:bg-neutral-950">
                  <div>
                    <div className="font-black text-primary-600 dark:text-primary-400 text-sm">{award.title}</div>
                    <div className="text-zinc-400 dark:text-neutral-400 font-semibold">{award.organization} &bull; {award.year}</div>
                  </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            {TESTIMONIALS.slice(0, 4).map((t, index) => (
              <div key={index} className="border-2 border-black dark:border-white p-6 bg-zinc-900/40 dark:bg-neutral-900 shadow-[6px_6px_0px_#000] dark:shadow-[6px_6px_0px_#fff] relative">
    ...
    return (
      <section className="py-28 bg-neutral-900 text-white relative overflow-hidden">
        ...
        <div className="max-w-5xl mx-auto px-6 relative z-10">
          ...
          <div className="bg-neutral-950/90 p-10 sm:p-16 rounded-3xl border border-neutral-800 shadow-2xl relative mb-12 backdrop-blur-2xl text-white">
            ...
            <div className="relative z-10 text-center space-y-8">
              ...
              <div>
                <h3 className="text-xl font-black text-white">{currentTestimonial.author}</h3>
            ...
            <div className="flex justify-between items-center mt-10 border-t border-neutral-800 pt-6">
              <button onClick={handlePrev} className="p-3 rounded-full bg-neutral-900 hover:bg-neutral-800 text-white border border-neutral-700 transition-colors flex items-center gap-2 text-xs font-bold px-5">
              ...
              <button onClick={handleNext} className="p-3 rounded-full bg-neutral-900 hover:bg-neutral-800 text-white border border-neutral-700 transition-colors flex items-center gap-2 text-xs font-bold px-5">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center text-xs text-neutral-400">
            {AWARDS.map((a, i) => (
              <div key={i} className="bg-neutral-950/60 border border-neutral-800/80 p-4 rounded-2xl flex flex-col items-center justify-center">
                <span className="text-xl mb-1">{a.logo}</span>
                <span className="font-extrabold text-neutral-300 text-xs">{a.title}</span>
  ...
  // ============================================================================
  return (
    <section className="py-20 bg-slate-50 dark:bg-neutral-950 text-white dark:text-neutral-100 relative overflow-hidden border-t border-zinc-800 dark:border-neutral-800">
      <div className="max-w-7xl mx-auto px-6">
        <div className="text-center max-w-3xl mx-auto mb-12">
          ...
          <p className="mt-3 text-sm text-zinc-400 dark:text-neutral-400 font-medium">
          ...
          {filters.map((f) => (
            <button
              ...
              className={`px-4 py-2 rounded-full text-xs font-extrabold transition-all ${
                selectedFilter === f ? 'btn-atomic-primary shadow-lg' : 'bg-zinc-900/60 dark:bg-neutral-900 text-zinc-300 dark:text-neutral-300 border border-zinc-800 dark:border-neutral-800 hover:border-primary-500'
              }`}
            >
              {f}
            </button>
          ...
          {filteredTestimonials.map((item, index) => (
            <div key={index} className="card-atomic-surface !p-6 sm:!p-8 flex flex-col justify-between transition-transform hover:-translate-y-1 shadow-md">
              <div>
                ...
                <p className="text-sm text-zinc-300 dark:text-neutral-300 leading-relaxed mb-6 italic">&ldquo;{item.text}&rdquo;</p>
              </div>
              <div className="border-t border-neutral-100 dark:border-neutral-800 pt-4 flex items-center justify-between">
                <div>
                  <h4 className="font-extrabold text-sm text-white dark:text-white">{item.author}</h4>
        ...
        {/* Awards Badges Deck */}
        <div className="bg-neutral-900 text-white p-8 sm:p-10 rounded-3xl border border-neutral-800 shadow-xl">
          <div className="text-center mb-8">
            <h3 className="text-lg sm:text-xl font-extrabold">Official Industry Recognitions &amp; Insurance Status</h3>
            <p className="text-xs text-neutral-400 mt-1">WSIB Licensed &bull; $1,000,000+ Zero-Deductible Cargo Insurance &bull; BBB Accredited</p>
          </div>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
            {AWARDS.map((award) => (
              <div key={award.id} className="bg-neutral-950 p-5 rounded-2xl text-center border border-neutral-800 hover:border-primary-500/50 transition-colors">
                ...
                <h4 className="text-sm font-black text-white">{award.title}</h4>
                <div className="text-xs text-neutral-400 mt-0.5">{award.organization} &bull; {award.year}</div>
              </div>
            ))}
          </div>
        </div>
