  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 px-4 pt-4 sm:px-6 lg:px-8 pointer-events-none">
      ...
      <nav className={`max-w-6xl mx-auto rounded-2xl pointer-events-auto transition-all duration-300 ${
        scrolled ? 'bg-neutral-950/95 shadow-2xl border border-neutral-800/80 py-3 px-6' : 'bg-neutral-900/80 backdrop-blur-xl border border-neutral-800 py-4 px-6 shadow-xl'
      }`}>
        <div className="flex items-center justify-between gap-6">
          ...
          >
            {BRAND.logoUrl ? (
              <div className="bg-zinc-900/60/95 px-3 py-1.5 rounded-xl shadow-md flex items-center justify-center max-w-[200px] hover:bg-zinc-900/60 transition-colors">
                <img src={BRAND.logoUrl} alt={BRAND.name} className="h-7 sm:h-8 w-auto object-contain" />
              </div>
            ) : (
              <div className="flex flex-col">
                <span className="text-xl sm:text-2xl font-black text-white tracking-wider uppercase font-['var(--font-heading)'] group-hover:text-primary-400 transition-colors">
                  {BRAND.name}
                </span>
              ...
              return (
                <button
                  ...
                  className={`text-xs uppercase tracking-wider transition-all relative py-1 ${
                    isActive ? 'text-primary-400 font-extrabold' : 'text-neutral-300 hover:text-white font-medium'
                  }`}
                >
          ...
          <div className="hidden md:flex items-center gap-4">
            <button
              ...
              className="text-xs text-neutral-300 hover:text-primary-400 font-semibold inline-flex items-center gap-1.5 px-3 py-2 rounded-lg hover:bg-neutral-800/50 transition-colors"
            >
            ...
            <a
              ...
              className="px-5 py-2.5 bg-primary-500 hover:bg-primary-600 text-white font-black text-xs uppercase tracking-wider rounded-lg shadow-lg flex items-center gap-2 transition-transform hover:scale-[1.02]"
            >
          ...
          <button
            ...
            className="xl:hidden p-2 text-white hover:text-primary-400 focus:outline-none"
        ...
        {/* Mobile Flyout */}
        {mobileOpen && (
          <div className="xl:hidden pt-6 pb-4 border-t border-neutral-800 mt-4 space-y-3">
            {navLinks.map((item) => (
              <button
                key={item.id}
                type="button"
                ...
                className={`block w-full text-left py-2 px-3 rounded-lg text-sm font-bold uppercase tracking-wider ${
                  activeTab === item.id ? 'bg-primary-500/20 text-primary-400' : 'text-neutral-300 hover:bg-neutral-800'
                }`}
              >
                {item.label}
              </button>
            ...
            <div className="pt-4 border-t border-neutral-800 flex flex-col gap-2">
              <a
                href={`tel:${BRAND.phoneRaw || BRAND.phone}`}
                className="w-full py-3 bg-primary-500 text-white font-black uppercase text-center rounded-lg flex items-center justify-center gap-2 shadow-lg"
              >
