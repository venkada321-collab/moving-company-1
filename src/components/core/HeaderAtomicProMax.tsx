...
const LogoCapsule: React.FC<{ compact?: boolean }> = ({ compact = false }) => {
  ...
  return (
    <div className={`flex items-center gap-3 bg-neutral-50 dark:bg-neutral-900 text-neutral-900 dark:text-white px-4 py-2 rounded-2xl shadow-lg border border-neutral-300 dark:border-neutral-700 hover:border-primary-400 transition-all cursor-pointer ${compact ? 'py-1.5 px-3' : ''}`}>
      <div className="w-8 h-8 rounded-xl bg-primary-500 text-neutral-900 dark:text-white flex items-center justify-center font-black font-mono text-base shrink-0 shadow-inner overflow-hidden">
        {BRAND.logoUrl && !imgError ? (
          <img 
            src={BRAND.logoUrl} 
            alt={BRAND.name} 
            onError={() => setImgError(true)}
            className="w-full h-full object-contain rounded-xl bg-white dark:bg-zinc-900/60 p-0.5" 
          />
        ) : (
          <span>{BRAND.logoSymbol || BRAND.name?.[0] || '★'}</span>
        )}
      </div>
      <span className={`font-black tracking-tight text-neutral-900 dark:text-white whitespace-nowrap overflow-hidden text-ellipsis max-w-[170px] sm:max-w-[220px] ${compact ? 'text-sm' : 'text-base'}`}>
...
export const HeaderAtomicProMax: React.FC<HeaderProps> = ({ activeTab, setActiveTab, onOpenCOIModal }) => {
      ...
      <header className="fixed top-4 left-0 right-0 z-50 px-4 sm:px-6 flex justify-center pointer-events-none">
        <div className="pointer-events-auto w-full max-w-6xl rounded-full bg-white/95 dark:bg-white dark:bg-neutral-950/95 text-neutral-900 dark:text-neutral-900 dark:text-white backdrop-blur-xl border border-neutral-200 dark:border-neutral-300 dark:border-neutral-700 shadow-2xl px-6 py-2.5 flex items-center justify-between transition-all duration-300 hover:border-primary-400/50">
          ...
          <nav className="hidden md:flex items-center gap-1 bg-neutral-100/80 dark:bg-neutral-800/80 rounded-full px-4 py-1.5 border border-neutral-200 dark:border-neutral-300 dark:border-neutral-700">
            {navLinks.map((link) => (
              <button
                ...
                className={`px-3.5 py-1 rounded-full text-xs font-black transition-all ${activeTab === link.id ? 'bg-primary-500 text-[color:var(--primary-contrast-text)] shadow-lg scale-105' : 'text-neutral-600 dark:text-neutral-200 hover:text-neutral-900 dark:hover:text-neutral-900 dark:text-white hover:bg-neutral-200/60 dark:hover:bg-neutral-700/60'}`}
              >
                {link.label}
              </button>
            ))}
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={() => setMobileMenuOpen(!mobileMenuOpen)} className="md:hidden p-2 text-neutral-900 dark:text-neutral-900 dark:text-white">
  ...
  if (variant === 'dual-ribbon-bar') {
    return (
      <header className="sticky top-0 z-50 w-full bg-white dark:bg-neutral-50 dark:bg-neutral-900 text-neutral-900 dark:text-neutral-900 dark:text-white shadow-2xl flex flex-col border-b border-neutral-200 dark:border-neutral-200 dark:border-neutral-800">
        <div className="bg-neutral-100 dark:bg-black text-neutral-600 dark:text-neutral-700 dark:text-neutral-300 px-6 py-2 flex justify-between items-center text-xs font-bold tracking-wide border-b border-neutral-200 dark:border-neutral-200 dark:border-neutral-800">
          <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-6">
            <span className="hidden md:inline text-neutral-600 dark:text-neutral-400">📍 {BRAND.hqAddress || 'Regional Operations'}</span>
    ...
    return (
      <header className="sticky top-0 z-50 bg-white dark:bg-neutral-50 dark:bg-neutral-900 text-neutral-900 dark:text-neutral-900 dark:text-white shadow-2xl">
        ...
        <div className="max-w-7xl mx-auto px-6 py-3 flex justify-between items-center border-b border-neutral-200 dark:border-neutral-200 dark:border-neutral-800">
          ...
          <button onClick={onOpenCOIModal} className="px-5 py-2 rounded-xl bg-neutral-100 dark:bg-neutral-800 hover:bg-neutral-200 dark:hover:bg-neutral-700 text-primary-600 dark:text-primary-400 font-bold text-xs border border-neutral-200 dark:border-neutral-300 dark:border-neutral-700 transition-colors shadow-sm inline-flex items-center gap-2">
