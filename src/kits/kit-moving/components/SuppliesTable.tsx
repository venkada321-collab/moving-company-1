...
export const SuppliesTable: React.FC<SuppliesTableProps> = ({ onSelectTab }) => {
  ...
  return (
    <section className="py-24 bg-zinc-900/60 text-white border-b border-zinc-800">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6 border-b border-zinc-800 pb-8">
          <div>
            ...
            <p className="text-zinc-400 font-medium text-sm mt-2 max-w-xl">
          ...
          <div className="flex bg-zinc-800 p-1 rounded-[var(--radius-button)] border border-neutral-300 self-start md:self-auto">
            <button
              ...
              className={`px-6 py-2.5 rounded-[var(--radius-button)] text-xs font-bold uppercase transition-all ${
                activeView === 'supplies' ? 'bg-neutral-950 text-white shadow' : 'text-zinc-400 hover:text-black'
              }`}
            >
              Packing Inventory
            </button>
            <button
              ...
              className={`px-6 py-2.5 rounded-[var(--radius-button)] text-xs font-bold uppercase transition-all ${
                activeView === 'storage' ? 'bg-neutral-950 text-white shadow' : 'text-zinc-400 hover:text-black'
              }`}
            >
              Storage Vaults
            </button>
        ...
        {activeView === 'supplies' ? (
          <div className="bg-zinc-900/40 border border-zinc-800 rounded-[var(--radius-card)] overflow-hidden shadow-sm">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-neutral-900 text-white text-xs uppercase font-bold tracking-wider">
                  ...
                  <tr key={item.id} className="hover:bg-zinc-900/60 transition-colors">
                    <td className="py-5 px-6">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <span className="font-extrabold block text-white">{item.name}</span>
                          <span className="text-[11px] text-primary-700 uppercase font-mono">{item.category} • Per {item.unit}</span>
                        </div>
                      </div>
                    </td>
                    <td className="py-5 px-6 hidden sm:table-cell text-zinc-400 text-xs font-normal max-w-md">
                      {item.description}
                    </td>
                    <td className="py-5 px-6 text-right font-mono font-black text-lg text-white">
                      ${item.price.toFixed(2)}
                    </td>
                    <td className="py-5 px-6 text-right">
                      <button
                        onClick={() => onSelectTab('quote')}
                        className="px-4 py-2 bg-neutral-900 hover:bg-primary-500 hover:text-white text-white text-xs uppercase font-black rounded-[var(--radius-button)] transition-colors inline-flex items-center gap-1 shadow-sm"
                      >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {STORAGE_OPTIONS.map((vault) => (
              <div key={vault.id} className="bg-zinc-900/60 border-2 border-zinc-800 rounded-[var(--radius-card)] p-8 flex flex-col justify-between hover:border-primary-500 transition-colors shadow-lg">
                <div>
                  <div className="flex justify-between items-center mb-4">
                    <span className="px-3 py-1 bg-primary-500/20 text-white text-[10px] font-black uppercase tracking-widest rounded-full">
                      {vault.dimensions}
                    </span>
                    <Shield className="w-5 h-5 text-primary-600" />
                  </div>
                  <h3 className="text-2xl font-black mb-2">{vault.name}</h3>
                  <p className="text-zinc-400 text-xs font-normal leading-relaxed mb-6">{vault.description}</p>
                  
                  <div className="bg-zinc-900/40 p-4 rounded-[var(--radius-button)] border border-zinc-800 mb-6">
                    <span className="text-xs text-neutral-500 block">Monthly Repossession Rate</span>
                    <span className="text-3xl font-black font-mono text-white">${vault.pricePerMonth}</span>
                    <span className="text-xs font-semibold text-emerald-700 block mt-1">✓ First 30 Days 50% Off Promotion</span>
                  </div>

                  <ul className="space-y-2 mb-8 text-xs font-bold text-zinc-300">
                ...
                <button
                  onClick={() => onSelectTab('quote')}
                  className="w-full py-3.5 bg-neutral-950 hover:bg-neutral-800 text-white font-black text-xs uppercase tracking-wider rounded-[var(--radius-button)] shadow-md flex items-center justify-center gap-2"
                >
